...
class Flask:
    ...
    def parseClasses(self, path):
        ...
        with open(path) as inp:
            ...
            for line in inp:
                number += 1
                m = Flask.COMMENT.search(line)
                if m:
                    continue

                m = Flask.WHITE.search(line)
                if m:
                    continue
    ...
    def parseSids(self, path):
        ...
        with open(path) as inp:
            ...
            for line in inp:
                ...
                m = Flask.SID.search(line)
                if m:
                    g = m.groupdict()
                    s = g["name"]
                    if s in sids:
                        raise DuplicateError(Flask.SID, path, number, s)
                    sids.append(s)
                    continue
    ...
    def parseVectors(self, path):
        ...
        with open(path) as inp:
            for line in inp:
                ...
                m = Flask.WHITE.search(line)
                if m:
                    if state == Flask.S_INHERIT:
                        state = Flask.S_NONE
                    continue

                m = Flask.COMMON.search(line)
                if m:
                    if state != Flask.S_NONE:
                        raise ParseError(Flask.COMMON, path, number)
                    g = m.groupdict()
                    c = g["name"]
                    if c in commons:
                        raise DuplicateError(Flask.COMMON, path, number, c)
                    commons.append(c)
                    common[c] = []
                    user_commons[c] = True
                    state = Flask.S_COMMON
                    continue
                ...
                if m:
                    ...
                    if c not in self.classes:
                        raise UndefinedError(Flask.CLASS, path, number, c)
                    vectors.append(c)
                    vector[c] = []
                    state = Flask.S_CLASS
                    continue
                ...
                if m:
                    ...
                    i = g["name"]
                    if c in inherits:
                        raise DuplicateError(Flask.INHERITS, path, number, c)
                    if i not in common:
                        raise UndefinedError(Flask.COMMON, path, number, i)
                    inherits[c] = i
                    state = Flask.S_INHERIT
                    if not self.userspace[c]:
                        user_commons[i] = False
                    continue

                m = Flask.OPENB.search(line)
                if m:
                    if (
                        state not in (Flask.S_CLASS, Flask.S_INHERIT, Flask.S_COMMON)
                    ) or state2 != Flask.S_NONE:
                        raise ParseError(Flask.OPENB, path, number)
                    state2 = Flask.S_OPEN
                    continue

                m = Flask.VECTOR.search(line)
                if m:
                    if state2 != Flask.S_OPEN:
                        raise ParseError(Flask.VECTOR, path, number)
                    g = m.groupdict()
                    v = g["name"]
                    if state in (Flask.S_CLASS, Flask.S_INHERIT):
                        if v in vector[c]:
                            raise DuplicateError(Flask.VECTOR, path, number, v)
                        vector[c].append(v)
                    elif state == Flask.S_COMMON:
                        if v in common[c]:
                            raise DuplicateError(Flask.VECTOR, path, number, v)
                        common[c].append(v)
                    continue

                m = Flask.CLOSEB.search(line)
                if m:
                    if state2 != Flask.S_OPEN:
                        raise ParseError(Flask.CLOSEB, path, number)
                    state = Flask.S_NONE
                    state2 = Flask.S_NONE
                    c = None
                    continue
        ...
        if Flask.S_NONE not in (state, state2):
            raise ParseError(Flask.EOF, path, number)
    ...
    def createAvPermissionsH(self, mode=USERSPACE):
        ...
        for common in self.commons:
            ...
            for p in self.common[common]:
                if not (mode == Flask.KERNEL and self.user_commons[common]):
    ...
    def createClassToStringH(self, mode=USERSPACE):
        ...
        if mode == Flask.KERNEL:
            results.append("    S_(NULL)\n")
        else:
            results.append('    S_("null")\n')

        for c in self.classes:
            if mode == Flask.KERNEL and self.userspace[c]:
                results.append("    S_(NULL)\n")
            else:
